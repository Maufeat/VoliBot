...
namespace voli {
  ...
  static void notifyUpdateStatus(const std::string status, voli::LeagueInstance& client, VoliServer& server) {
    client.currentStatus = status;
    auto x = lol::GetLolSummonerV1CurrentSummoner(client);
    auto y = lol::GetLolLoginV1Wallet(client);
    const json settings = { { "autoPlay", client.account.status },{ "queue", client.account.queueId } };
